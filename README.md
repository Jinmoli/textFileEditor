...
- 提供高亮能力矩阵文档，便于查看 `xml / yaml / sql / shell / batch / ps1` 的现状与治理建议：[`docs/highlighting/text-file-editor-highlighting-matrix.md`](docs/highlighting/text-file-editor-highlighting-matrix.md)。
- 支持 `Ctrl/Cmd+S` 快捷保存。
- 支持 `Ctrl/Cmd+F` 查找，并提供中文查找/替换面板。
- 支持按文件记忆只读模式和自动换行状态，避免一个文件的编辑状态影响另一个文件。
- 支持文件大小、最后修改时间、编码、换行风格、语言模式、光标行列状态显示。
...
- 推荐使用 `Ctrl/Cmd+F` 打开当前文件的查找/替换面板。
...
- `高亮支持`：`xml`、`yaml`、`sql`、`json`、`html`、`java`、`properties`、`sh/bash/zsh`、`bat/cmd`、`ps1`、`toml`、`dockerfile`、`gradle`、`js/ts`、`css` 等格式可直接阅读；脚本类高亮覆盖常见控制流、变量、函数或标签结构。
- `格式化支持`：当前仅内置 `JSON / XML / YAML / SQL` 格式化；其他格式暂不自动改写内容结构。
- `编码与换行`：支持常见编码重新读取，状态栏会显示 `LF / CRLF / 混合换行`；检测到混合换行时会给出提醒。
- `建议谨慎编辑`：非 `UTF-8/ASCII` 的旧编码文件、混合换行文件、超大日志/SQL、以及需要精确保持脚本语义的文件，建议先只读检查，再决定是否保存或格式化。
...
推送形如 `v1.2.0` 的 tag 后，GitHub Actions 会自动运行测试、类型检查、构建，并上传插件 zip 到 GitHub Release。
